    ...
    public class ConsoleWindow : EditorWindow, IHasCustomMenu
    {
        ...
        internal class Constants
        {
            ...
            public static void Init()
            {
                ...
                LogSmallStyle.normal.background = null;
                LogSmallStyle.onNormal.background = null;
                WarningSmallStyle.normal.background = null;
                WarningSmallStyle.onNormal.background = null;
                ErrorSmallStyle.normal.background = null;
                ErrorSmallStyle.onNormal.background = null;
                IconLogSmallStyle.normal.background = null;
                IconLogSmallStyle.onNormal.background = null;
                IconWarningSmallStyle.normal.background = null;
                IconWarningSmallStyle.onNormal.background = null;
                IconErrorSmallStyle.normal.background = null;
                IconErrorSmallStyle.onNormal.background = null;
                ...
                foreach (ListViewElement el in ListViewGUI.ListView(m_ListView, Constants.Box))
                {
                    ...
                    else if (e.type == EventType.Repaint)
                    {
                        ...
                        if (Constants.LogStyleLineCount == 1)
                        {
                            ...
                            rt.width = 16f;
                            rt.height = 16f;
                            GUI.DrawTexture(rt, GetIconForErrorMode(mode, false));
                        }
                        else
                        {
        ...
        private void SearchField(Event e)
        {
            ...
            var filteringText = EditorGUI.ToolbarSearchField(0, rect, searchText, false);
        ...
        public void AddItemsToMenu(GenericMenu menu)
        {
            if (Application.platform == RuntimePlatform.OSXEditor)
                menu.AddItem(EditorGUIUtility.TempContent("Open Player Log"), false, UnityEditorInternal.InternalEditorUtility.OpenPlayerConsole);
            menu.AddItem(EditorGUIUtility.TempContent("Open Editor Log"), false, UnityEditorInternal.InternalEditorUtility.OpenEditorConsole);

            menu.AddItem(EditorGUIUtility.TempContent("Show Timestamp"), HasFlag(ConsoleFlags.ShowTimestamp), SetTimestamp);
        ...
        private void AddStackTraceLoggingMenu(GenericMenu menu)
        {
            ...
            foreach (LogType logType in Enum.GetValues(typeof(LogType)))
            {
                foreach (StackTraceLogType stackTraceLogType in Enum.GetValues(typeof(StackTraceLogType)))
                {
                    ...
                    menu.AddItem(EditorGUIUtility.TempContent("Stack Trace Logging/" + logType + "/" + stackTraceLogType), PlayerSettings.GetStackTraceLogType(logType) == stackTraceLogType,
            ...
            foreach (StackTraceLogType stackTraceLogType in Enum.GetValues(typeof(StackTraceLogType)))
            {
                menu.AddItem(EditorGUIUtility.TempContent("Stack Trace Logging/All/" + stackTraceLogType), (StackTraceLogType)stackTraceLogTypeForAll == stackTraceLogType,
